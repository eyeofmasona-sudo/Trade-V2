...
const MarketRow = React.memo(({ symbol, handleTrade, t }: { symbol: string, handleTrade: (s:string)=>void, t: any }) => {
  const price = useTradingStore(s => s.prices[symbol] || 0);
  const change = useTradingStore(s => s.priceChanges[symbol] || 0);
  ...
  return (
    <tr className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
      ...
      <td className="px-6 py-4 font-mono text-slate-200">
        {price > 0 ? price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 5 }) : '...'}
      </td>
      <td className="px-6 py-4">
