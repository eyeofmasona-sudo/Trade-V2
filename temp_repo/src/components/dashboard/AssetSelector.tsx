...
export const AssetSelector: React.FC = () => {
  ...
  const currentPrice = prices[currentPair] || 0;
  const currentChange = priceChanges[currentPair] || 0;
    ...
    <div className="relative z-50 h-full flex items-center" ref={dropdownRef}>
      ...
      >
        ...
        <div className="hidden md:flex flex-col items-end opacity-80">
            <span className="text-xs font-mono font-bold text-white">{currentPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 5 })}</span>
            <span className={`text-[9px] font-bold flex items-center gap-0.5 ${isPositive ? 'text-accent-secondary' : 'text-accent-quaternary'}`}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right">
                         <div className="text-xs font-mono font-bold text-white">{p ? p.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 5 }) : '---'}</div>
                         <div className={`text-[10px] font-bold font-mono ${isPos ? 'text-accent-secondary' : 'text-accent-quaternary'}`}>
                           {isPos ? '+' : ''}{c.toFixed(2)}%
                         </div>
                      </div>
                    </button>
